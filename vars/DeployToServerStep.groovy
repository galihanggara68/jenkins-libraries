/*
    ...
    parameters :
        credentialsId : Credentials id used to login to server
        driverLetter : Drive Letter to use to mount deployment path
...
def call(Map config = [:]){
    script{
        withCredentials([string(credentialsId: "${config.credentialsId}", passwordVariable: 'PASS', usernameVariable: 'USER')]) {
            try{
